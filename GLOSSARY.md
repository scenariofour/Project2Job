...
| **Skill** | A reusable, host-native method for routing and performing one Project2Job analysis. | The Context Registry or Agent. Skill instructions do not themselves monitor change or orchestrate future runs. | `docs/05_SKILL_PRODUCT_SPEC.md` |
| **Context Registry** | Consent-gated local infrastructure shared by the Project2Job Skills. It stores minimal Project, JD, and Analysis Run identity, fingerprints, confirmed facts, provenance boundaries, gaps, and output references so compatible work can be reused across sessions. | A user-facing Skill, a general candidate database, or the Agent. It does not monitor, decide, or regenerate in the background. | `skill/p2j/references/context-registry.md` |
| **Agent** | The stateful product runtime that resolves saved context, observes Project/JD/correction changes, selects a bounded action, maintains dependencies, and updates affected outputs with a change explanation. V0 is implemented and deterministically tested; live-model and target-user quality remain unvalidated. | A Skill, the passive Context Registry, or the Day 1 per-claim loop. | `docs/06_AGENT_PRODUCT_SPEC.md` |
